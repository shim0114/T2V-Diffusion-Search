...
class MotionSmoothness:
    ...
    def motion_score(self, video_tensor):
        ...
        frames = video_tensor.permute(0, 2, 3, 1).cpu().numpy().clip(0, 255).astype(np.uint8)
...
def motion_smoothness(motion, video_tensor):
    # sim = []
    # video_results = []
    # for video_path in tqdm(video_list, disable=get_rank() > 0):
    score_per_video = motion.motion_score(video_tensor)
...
def compute_motion_smoothness(video_tensor, device, submodules_list, **kwargs):
    ...
    motion_smoothness_score = motion_smoothness(motion, video_tensor)
    # all_results, video_results = motion_smoothness(motion, video_list)
    # if get_world_size() > 1:
    #     video_results = gather_list_of_dict(video_results)
    #     all_results = sum([d['video_results'] for d in video_results]) / len(video_results)
    return motion_smoothness_score # all_results, video_results


if __name__ == "__main__":
    # json_dir = 'overall_consistency.json'
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    CUR_DIR = "verifiers/VBench/vbench"
    ...
    # import subprocess
    # wget_command = ['wget', '-P', f"{CACHE_DIR}/amt_model",
    #                 'https://huggingface.co/lalala125/AMT/resolve/main/amt-s.pth']
    # subprocess.run(wget_command, check=True)
    video_tensor = load_video('results_sampling/a cat wearing sunglasses and working as a lifeguard at pool.-0000.mp4')
    motion_smoothness_score = compute_motion_smoothness(video_tensor, device, submodules_list)
    print(motion_smoothness_score)