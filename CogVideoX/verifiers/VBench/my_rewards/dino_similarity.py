...
def subject_consistency(model, video_tensor, device, read_frame=False):
    # sim = 0.0
    # cnt = 0
    # video_results = []
    if read_frame:
    ...
    if read_frame:
        ...
        images = video_tensor # load_video(video_path)
    ...
    return sim_per_images # sim_per_frame, video_results

def compute_subject_consistency(video_tensor, device, submodules_list, **kwargs):
    ...
    subject_consistency_score = subject_consistency(dino_model, video_tensor, device, read_frame)
    # if get_world_size() > 1:
    #     video_results = gather_list_of_dict(video_results)
    #     all_results = sum([d['video_results'] for d in video_results]) / len(video_results)
    return subject_consistency_score # all_results, video_results

if __name__ == '__main__':
    # json_dir = 'subject_consistency.json'
    ...
    video_tensor = load_video('results_sampling/best-of-n/devil_video_low_64/Old man standing up with his hand on heart during the national anthem at an international sport event or football competition. close-up.-0000.mp4')
    subject_consistency_score = compute_subject_consistency(video_tensor, device, submodules_list)
    print(subject_consistency_score)
