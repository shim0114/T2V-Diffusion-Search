...
def _parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./config/devil_very_high.yaml",
        help="Path to config.")
    parser.add_argument(
        "--num_runs",
        type=int,
        default=1,
        help="Number of times to run inference per prompt (default: 1). Only for Vanilla Inference")
    parser.add_argument(
        "--use_dlbs",
        action="store_true",
...
def generate(args):
    ...
    if not "t2v" in args.task:
        ...
        args.weights_list = weights_list 
        args.prompt_list = prompt_list
        
        # rank0 だけがReward Modelを作る
        if rank == 0:
            ...
            compute_video_reward = None

        
        if args.use_dlbs:
            wan_t2v = wan.WanT2V_DLBS(
                config=cfg,
            ...
            for run_idx in range(args.num_runs):
                for prompt in prompt_list:
                    ...
                    if rank == 0:
                        ...
                        #     suffix = '.png' if "t2i" in args.task else '.mp4'
                        #     args.save_file = f"{args.task}_{args.size.replace('*','x') if sys.platform=='win32' else args.size}_{args.ulysses_size}_{args.ring_size}_{formatted_prompt}_{formatted_time}" + suffix
                        run_idx_str = '%04d' % run_idx
                        save_file = f'{args.save_img_path}/{prompt}-{run_idx_str}.mp4'
