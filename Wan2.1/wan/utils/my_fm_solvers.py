...
class FlowDPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def dpm_solver_first_order_update(
        ...
        **kwargs,
        ...
        # x_original = (sample - sigma_s * model_output) / alpha_s
        ...
        # added
        return x_t, None # pyright: ignore
    ...
    def multistep_dpm_solver_second_order_update(
        ...
        **kwargs,
        ...
        # added
        # x_original = (sample - sigma_s0 * model_output_list[-1]) / alpha_s0
    ...
    def step(
        ...
        Args:
            ...
            num_candidates (`int`, defaults to 1):
                Number of different noise draws to generate (and hence different candidate samples).
        Returns:
        ...
        # We'll store the candidate results in lists
        candidate_prev_samples = []
        ...
        for candidate_idx in range(num_candidates):
            ...
            # print(self.config.solver_order)
            ...
            if self.config.solver_order == 1 or self.lower_order_nums < 1 or lower_order_final:
                prev_sample, _ = self.dpm_solver_first_order_update(
                    model_output, sample=sample, noise=noise)
            elif self.config.solver_order == 2 or self.lower_order_nums < 2 or lower_order_second:
                # added
                prev_sample, _ = self.multistep_dpm_solver_second_order_update(
            ...
            # Accumulate results
            candidate_prev_samples.append(prev_sample)
        ...
        if not return_dict:
            # tmp
            return candidate_prev_samples # (prev_sample, )
        else:
            raise NotImplementedError()
    ...
    def my_step(self,
        ...
        sigma_t = prev_timestep.to('cpu') / self.config.num_train_timesteps
        sigma_s = timestep.to('cpu') / self.config.num_train_timesteps
        
        # convert model output
        # model_output = self.convert_model_output(model_output, sample=sample)
        # DPM-Solver++ needs to solve an integral of the data prediction model.
        if self.config.prediction_type == "flow_prediction":
            x0_pred = sample - sigma_s * model_output
        else:
            raise ValueError(
                f"prediction_type given as {self.config.prediction_type} must be one of `epsilon`, `sample`,"
                " `v_prediction`, or `flow_prediction` for the FlowDPMSolverMultistepScheduler."
